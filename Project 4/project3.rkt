...
(define (fpG g)
  (first (G-Ps g)))
(define (ispawn? p)
  (member (P-reach p) (list PawnW PawnB)))

;;Given a normal time, give back time for pawns to attack from
(define (pawnify-time g p time)
  ...
  (define (inbounds? pos)
    (let* ([dems (G-dem g)]
           [accx (range 1 (+ 1 (first dems)))]
           [accy (range 1 (+ 1 (second dems)))]
           [x (first pos)]
           [y (second pos)])
      (and (member x accx)
           (member y accy))))
  (if (not (ispawn? p))
      ...
      (if (empty? time)
          time
          (filter notempty? (foldl (λ(t acc) (let* ([ttime (second t)]
                                                    [tpos (first t)]
                                                    [tx (first tpos)]
                                                    [ty (second tpos)]
                                                    [py (pawny t)])
                                               (cons (if (inbounds? (list (- tx 1) py))
                                                         (list (list (- tx 1) py) (- ttime 1) (first t))
                                                         empty)
                                                     (cons (if (inbounds? (list (+ tx 1) py))
                                                               (list (list (+ tx 1) py) (- ttime 1) (first t))
                                                               empty)
                                                           acc)))) empty time)))))
(define (pawn-ap g p t)
  (define (pawny pos)
    (if (equal? (P-reach p) PawnW)
        (+ (second pos) 1)
        (- (second pos) 1)))
                                    ...
                                    (let ([tra2 (filter (λ (t) (not (check-duplicates
                                      ...
                                      (if (empty? tra2)
                                          ...
                                          (let* ([tra2 (first tra2)] ;only do the first trajectory
                                                 ;;--SPECIAL CASE: Pawn
                                                 [pawntra (λ (tr) (append tr (list (third t))))]
                                                 [tra2 (if (ispawn? p)
                                                           (pawntra tra2)
                                                           tra2)]
